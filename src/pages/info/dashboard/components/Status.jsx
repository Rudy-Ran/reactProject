...
function Status(props) {
    ...
    const { healthInfo, alarmInfo, uidStatus, powerStatus, tcgStatus } = useMappedState(mapState);
    return (
        <div className="dashboard">
            <Row>
                ...
                <Col span={6}>
                    {/* 初始化UID状态 */}
                    {initUIDStatus(uidStatus)}
                    {/* 初始化服务器电源状态 */}
                    {initPowerStatus(powerStatus)}
                    {/* 初始化TPM/TCM状态 */}
                    {initTCGStatus(tcgStatus)}
                    <Row>
...
function handleAllHealth(healthInfo) {
    if (healthInfo.cc === 0) {
        return (
            <Col span={12}>
                {/* 服务器总体健康状态 */}
...
function initUIDStatus(uidStatus) {
    let uidTxt;
    let uidImgURL;
    switch (uidStatus) {
        case 0: {
...
function initPowerStatus(powerStatus) {
    let powerTxt;
    let powerImgURL;
    powerStatus ? (powerTxt = '关闭', powerImgURL = powerImg1) : (powerTxt = '开启', powerImgURL = powerImg2);
    return (
        <Row className="led-info">
...
function initTCGStatus(tcgStatus) {
    let status;
...
function deviceCount(healthInfo) {
    let count = 0;
